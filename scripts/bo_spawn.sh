...
for b in `ls ML-DPC/ChampSimTraces`; do for t in `ls ML-DPC/ChampSimTraces/$b`; do
    ...
    export BUCKET=ip
    export EPOCHS=20
    GENERATED_DIR="generated-debugcnn-$LOOKAHEAD"
    ...
    if [[ $SUCCEEDED = 0 ]]; then
      ...
      sbatch --dependency=afterok:$JOB_ID --export=ALL --mem 10G --time 6:00:00 scripts/single.sh ./ml_prefetch_sim.py run ML-DPC/ChampSimTraces/$b/$t --num-prefetch-warmup-instructions 100 --num-instructions 100 --results-dir="results-debugcnn-$LOOKAHEAD" --prefetch $PREFETCH_FILE
    fi
